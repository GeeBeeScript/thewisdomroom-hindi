...
export const createTeaching = async ({
  ...
  await db.teachingsHindi.create({
    data: {
      title: title,
...
export const getTeaching = async (id: string) => {
  ...
  try {
    particularTeaching = await db.teachingsHindi.findUniqueOrThrow({
      where: {
        id: id,
      },
    });
...
export const updateTeaching = async ({
  ...
  await db.teachingsHindi.update({
    where: {
      id: id,
    },
    data: {
...
export const getTeachingQty = async (quantity: number) => {
  try {
    const fiveTeachings = await db.teachingsHindi.findMany({
      take: quantity,
      orderBy: {
        updatedAt: "desc",
      },
...
export const deleteTeaching = async (id: string) => {
  await db.teachingsHindi.delete({
    where: {
      id: id,
    },
  });
